...
static __device__ __forceinline__ void flash_attn_tile_load_tile_quant(
        const char * const __restrict__ KV, half2 * const __restrict__ tile_KV, const int stride_KV, const int i_sup, const int col_offset) {
    constexpr int cpy_nb = ggml_cuda_get_max_cpy_bytes();
    constexpr int cpy_ne = cpy_nb / 4;
    static_assert(J % cpy_ne == 0, "bad J");
    ...
    const int chunks_per_row = J / cpy_ne;
    const int total_chunks = I * chunks_per_row;
    const int row_stride = J + 2*J_padding; // in half units

    half * tile = reinterpret_cast<half *>(tile_KV);

    for (int idx = tid; idx < total_chunks; idx += nthreads) {
        const int i = idx / chunks_per_row;
        const int j = (idx - i*chunks_per_row) * cpy_ne;

        __align__(16) half tmp[cpy_ne];
        if (!oob_check || i < i_sup) {
            dequantize(KV + int64_t(i)*stride_KV, tmp, int64_t(col_offset + j));
        } else {
#pragma unroll
            for (int l = 0; l < cpy_ne; ++l) {
                tmp[l] = __float2half(0.0f);
            }
        }

        ggml_cuda_memcpy_1<cpy_ne*sizeof(half)>(tile + i*row_stride + j, tmp);
    }
}

template<ggml_type type_KV, int warp_size, int nwarps, int I, int J, int J_padding, bool oob_check>
static __device__ __forceinline__ void flash_attn_tile_load_tile_quant(
        const char * const __restrict__ KV, float * const __restrict__ tile_KV, const int stride_KV, const int i_sup, const int col_offset) {
    constexpr int cpy_nb = ggml_cuda_get_max_cpy_bytes();
    constexpr int cpy_ne = cpy_nb / 4;
    static_assert(J % cpy_ne == 0, "bad J");
    static_assert(type_KV == GGML_TYPE_Q4_0 || type_KV == GGML_TYPE_Q8_0, "unsupported quant type");
    ...
    const int chunks_per_row = J / cpy_ne;
    const int total_chunks = I * chunks_per_row;
    const int row_stride = J + J_padding; // in float units

    for (int idx = tid; idx < total_chunks; idx += nthreads) {
        const int i = idx / chunks_per_row;
        const int j = (idx - i*chunks_per_row) * cpy_ne;

        __align__(16) float tmp[cpy_ne];
        if (!oob_check || i < i_sup) {
            dequantize(KV + int64_t(i)*stride_KV, tmp, int64_t(col_offset + j));
        } else {
#pragma unroll
        ...
        ggml_cuda_memcpy_1<cpy_nb>(tile_KV + i*row_stride + j, tmp);
    }
}
...
static __device__ __forceinline__ void flash_attn_tile_iter_KQ(
    ...
    if constexpr (type_K == GGML_TYPE_F16) {
        ...
        flash_attn_tile_load_tile_quant<type_K, warp_size, nwarps, nbatch_fa, nbatch_K, cpy_ne, oob_check>
            (K + int64_t(k_VKQ_0)*stride_K2, KV_tmp, stride_K2, k_VKQ_sup, k_KQ_0);
    }
    __syncthreads();
...
template <int warp_size, int nwarps, int ncols1, int ncols2, int DKQ, int DV, int nbatch_fa, int nbatch_K,
    bool use_logit_softcap, bool oob_check, ggml_type type_K, ggml_type type_V, typename T_vec_dot, typename T_KQ, typename T_acc>
...
#pragma unroll
    for (int k0 = 0; k0 < nbatch_fa; k0 += nbatch_V) {
        if constexpr (type_V == GGML_TYPE_F16) {
            ...
            flash_attn_tile_load_tile_quant<type_V, warp_size, nwarps, nbatch_V, DV, 0, oob_check>
                (V + int64_t(k_VKQ_0 + k0)*stride_V2, KV_tmp, stride_V2, k_VKQ_sup - k0, 0);
        }
        __syncthreads();
...
template<int DKQ, int DV, int ncols1, int ncols2, ggml_type type_K, ggml_type type_V, bool use_logit_softcap> // D == head size
...
static __global__ void flash_attn_tile(
    ...
    const char  * K_h  = (const char  *) (K + nb13*sequence + nb12*(head0 / gqa_ratio));
    const char  * V_h  = (const char  *) (V + nb23*sequence + nb22*(head0 / gqa_ratio)); // K and V have same shape

    const half * maskh = mask ? (const half *) (mask + nb33*(sequence % ne33)) : nullptr;

    const int stride_K2   = type_K == GGML_TYPE_F16 ? nb11 / sizeof(half2) : nb11;
    const int stride_V2   = type_V == GGML_TYPE_F16 ? nb21 / sizeof(half2) : nb21;
    ...
    if (ncols2 == 1) {
        ...
        while (k_VKQ_0 < k_VKQ_max - nbatch_fa) {
            ...
            flash_attn_tile_iter<warp_size, nwarps, ncols1, ncols2, DKQ, DV, nbatch_fa, nbatch_K, use_logit_softcap, oob_check, type_K, type_V>
                (Q_tmp, K_h, V_h, maskh, ne01, logit_softcap, slope, KQ, KV_tmp,
...
template <int DKQ, int DV, int ncols2, ggml_type type_K, ggml_type type_V, bool use_logit_softcap>
static void launch_fattn_tile_switch_ncols1(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if constexpr (DV <= 128) {
        if (Q->ne[1] > 32/ncols2) {
            ...
            fattn_kernel_t fattn_kernel = flash_attn_tile<DKQ, DV, cols_per_block/ncols2, ncols2, type_K, type_V, use_logit_softcap>;
...
template <int DKQ, int DV, ggml_type type_K, ggml_type type_V, bool use_logit_softcap>
static void launch_fattn_tile_switch_ncols2(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if constexpr (DV == 512) {
        if (use_gqa_opt && gqa_ratio % 16 == 0) {
            launch_fattn_tile_switch_ncols1<DKQ, DV, 16, type_K, type_V, use_logit_softcap>(ctx, dst);
            return;
        }
        if (use_gqa_opt && gqa_ratio % 4 == 0) {
            launch_fattn_tile_switch_ncols1<DKQ, DV, 4, type_K, type_V, use_logit_softcap>(ctx, dst);
            return;
        }
    }
    ...
    if constexpr (DV <= 256) {
        if (use_gqa_opt && gqa_ratio % 8 == 0) {
            launch_fattn_tile_switch_ncols1<DKQ, DV, 8, type_K, type_V, use_logit_softcap>(ctx, dst);
            return;
        }
        ...
        if (use_gqa_opt && gqa_ratio % 2 == 0) {
            launch_fattn_tile_switch_ncols1<DKQ, DV, 2, type_K, type_V, use_logit_softcap>(ctx, dst);
            return;
        }

        launch_fattn_tile_switch_ncols1<DKQ, DV, 1, type_K, type_V, use_logit_softcap>(ctx, dst);
        return;
    }
    GGML_ABORT("fatal error");
}
    ...
    if (logit_softcap == 0.0f) {
        ...
        if constexpr (DKQ == 576 && DV == 512) {
            if (K->type == GGML_TYPE_Q8_0 && V->type == GGML_TYPE_Q8_0) {
                launch_fattn_tile_switch_ncols2<DKQ, DV, GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, use_logit_softcap>(ctx, dst);
                return;
            }
            if (K->type == GGML_TYPE_Q4_0 && V->type == GGML_TYPE_Q4_0) {
                launch_fattn_tile_switch_ncols2<DKQ, DV, GGML_TYPE_Q4_0, GGML_TYPE_Q4_0, use_logit_softcap>(ctx, dst);
                return;
            }
        }
        launch_fattn_tile_switch_ncols2<DKQ, DV, GGML_TYPE_F16, GGML_TYPE_F16, use_logit_softcap>(ctx, dst);
    } else {

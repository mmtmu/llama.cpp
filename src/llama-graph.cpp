...
ggml_tensor * llm_graph_context::build_ffn(
   ...
   llm_ffn_gate_type   type_gate,
    ...
    if (down) {
        ...
        if (arch == LLM_ARCH_GLM4 || arch == LLM_ARCH_GLM4_MOE || arch == LLM_ARCH_GLM_DSA || arch == LLM_ARCH_JAIS2) {
            // GLM4, GLM4_MOE, GLM_DSA, and JAIS2 seem to have numerical issues with half-precision accumulators
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    if (wo) {
        cur = build_lora_mm(wo, cur);
        if (arch == LLM_ARCH_GLM4 || arch == LLM_ARCH_GLM4_MOE || arch == LLM_ARCH_GLM_DSA) {
            // GLM4, GLM4_MOE, and GLM_DSA seem to have numerical issues with half-precision accumulators
            ggml_mul_mat_set_prec(cur, GGML_PREC_F32);
        }
    }
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    const auto & kq_mask_f32 = inp->self_kq_mask_mla;
    ...
    top_k = ggml_view_4d(ctx0, top_k, top_k->ne[0], top_k->ne[1], top_k->ne[3], 1,

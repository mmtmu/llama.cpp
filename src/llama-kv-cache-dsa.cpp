...
size_t llama_kv_cache_dsa::seq_write_range(llama_io_write_i & io, llama_seq_id seq_id, llama_pos p0, llama_pos p1) const {
    const size_t n0 = kv_mla->seq_write_range(io, seq_id, p0, p1);
    if (n0 == 0) {
        return 0;
    }

    const size_t n1 = kv_lid->seq_write_range(io, seq_id, p0, p1);
    if (n1 == 0) {
        return 0;
    }
...
size_t llama_kv_cache_dsa::seq_read_range(llama_io_read_i & io, llama_seq_id seq_id, llama_pos pos_shift) {
    const size_t n0 = kv_mla->seq_read_range(io, seq_id, pos_shift);
    if (n0 == 0) {
        return 0;
    }

    const size_t n1 = kv_lid->seq_read_range(io, seq_id, pos_shift);
    if (n1 == 0) {
        return 0;
    }
